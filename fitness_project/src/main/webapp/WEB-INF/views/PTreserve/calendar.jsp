...
<body>
    ...
    <script>
        ...
        $(function(){
        	...
        	{
        		$(".fc-col-header").find(".fc-day-sun div a").text("일");
        		$(".fc-col-header").find(".fc-day-mon div a").text("월");
        		$(".fc-col-header").find(".fc-day-tue div a").text("화");
        		$(".fc-col-header").find(".fc-day-wed div a").text("수");
        		$(".fc-col-header").find(".fc-day-thu div a").text("목");
        		$(".fc-col-header").find(".fc-day-fri div a").text("금");
        		$(".fc-col-header").find(".fc-day-sat div a").text("토");
        		
        		$(".fc-col-header").find(".fc-col-header-cell").css("background-color","#454545");
        		$(".fc-col-header").find("a").css("color","whitesmoke");
        	}
            ...
            // 마우스 클릭의 이벤트 순서   mousedown -> mouseup -> click
            {
	        	...
	        	$("#calendar").on("mousedown", "td.fc-daygrid-day", function(e) {
					...
					if($(this).hasClass("select-cell")) {
						...
						//선택한 날짜 모달 타이틀에 출력
						$(".tModal-h2").text("${param.trainerName} 트레이너 " + " 예약 date: " + $(this).data("date"));
						
						// 모달 켜짐과 애니메이션 실행
		   	    		$("#tModal.tModal-overlay").css({"display":"flex", "animation":"fadein "+ animTime + "ms"});
		   	    		
		   	    		// 모달창이 켜지면 백그라운드의 스크롤 막기 (부작용으로 스크롤바가 사라진 만큼 body의 width값이 커짐)
			        	...
			        	$(".tModal-input").css("width", $(".tModal-h2").width());
		   	    		$(".tModal-content").css("width", $(".tModal-table").width() + 10);
					}
	   	    	});// END of 모달 open
            ...
            $(".reserve").click(function(){
            	alert("예약기능 구현 준비중 입니다. 감사합니다.");
            });
            
        });// END of Ready
    </script>
    <script>
	    function teach_CellEvent(){
	    	/* f = $("div.fc-daygrid-body.fc-daygrid-body-unbalanced.fc-daygrid-body-natural").mousedown
	        $("#calendar").on("click", "td.fc-daygrid-day", function() {
	            $("div.fc-daygrid-body.fc-daygrid-body-unbalanced.fc-daygrid-body-natural").off();
	        }).on("dblclick", "td.fc-daygrid-day", function() {
	        	console.log("날짜 클릭");
		    		console.log($(this).data("date"));
		    		$("#tModal.modal-overlay").css("display", "flex");
		    		$("div.fc-daygrid-body.fc-daygrid-body-unbalanced.fc-daygrid-body-natural").mousedown(f).mousedown();
	        }) */
	        
	        // 날짜 클릭시 시간표 모달창 출력
	        //$("#calendar").off("mousedown", "td.fc-daygrid-day ");
	    	//$("#calendar").on("click", "td.fc-daygrid-day .fc-daygrid-day-frame .fc-highlight", function(){
	    }
    </script>
</body>
</html>
